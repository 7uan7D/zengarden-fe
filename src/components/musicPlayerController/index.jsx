...
const handleTogglePlay = (setPlaying) => {
    if (globalAudioState.audio) {
        ...
        loadAudioPlayer(globalAudioState.currentIndex, setPlaying, globalAudioState.setCurrentIndex);
    }
};

// Hàm chuyển về bài trước
const handlePrevious = (setPlaying, setCurrentIndex) => {
    const newIndex = (globalAudioState.currentIndex - 1 + playlist.length) % playlist.length;
...
export default function MusicPlayerController({ positionClass = "relative", setPlaying, setCurrentIndex }) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-2 justify-center">
                <Button variant="outline" onClick={() => handlePrevious(setPlaying, setCurrentIndex)}>
                ...
                <Button variant="outline" onClick={() => handleNext(setPlaying, setCurrentIndex)}>
                ...
                <ul className="space-y-2">
                    {playlist.map((track, index) => (
                        <li
                            ...
                            onClick={() => loadAudioPlayer(index, setPlaying, setCurrentIndex)}
                        >
                            {track.title}
                        </li>
                    ))}
